...
export default function Library() {
  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold tracking-tight">Library Management</h2><p className="text-muted-foreground">Manage book catalog, issue/return, and fines.</p></div>
      <Tabs defaultValue="catalog">
        <TabsList><TabsTrigger value="catalog"><BookOpen className="w-4 h-4 mr-1" />Catalog</TabsTrigger><TabsTrigger value="issued"><BookCheck className="w-4 h-4 mr-1" />Issue / Return</TabsTrigger></TabsList>
...
function BookCatalog() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Table><TableHeader><TableRow><TableHead>Title</TableHead><TableHead>Author</TableHead><TableHead>ISBN</TableHead><TableHead>Category</TableHead><TableHead>Shelf</TableHead><TableHead>Available</TableHead><TableHead>Status</TableHead><TableHead></TableHead></TableRow></TableHeader>
          <TableBody>{isLoading ? <TableRow><TableCell colSpan={8} className="text-center">Loading...</TableCell></TableRow> : books?.length === 0 ? <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground">No books.</TableCell></TableRow> : books?.map(b => (
            <TableRow key={b.id}><TableCell className="font-medium">{b.title}</TableCell><TableCell>{b.author}</TableCell><TableCell>{b.isbn || '-'}</TableCell><TableCell><Badge variant="secondary">{b.category}</Badge></TableCell><TableCell>{b.shelfLocation || '-'}</TableCell><TableCell>{b.availableCopies}/{b.totalCopies}</TableCell><TableCell><Badge variant={b.availableCopies > 0 ? 'default' : 'destructive'}>{b.availableCopies > 0 ? 'Available' : 'All Issued'}</Badge></TableCell><TableCell><Button variant="ghost" size="sm" onClick={() => deleteM.mutate({ id: b.id }, { onSuccess: () => qc.invalidateQueries({ queryKey: getListLibraryBooksQueryKey() }) })}><Trash2 className="w-4 h-4" /></Button></TableCell></TableRow>
          ))}</TableBody></Table>
      </CardContent>
    </Card>
  );
...
function IssuedBooks() {
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between"><CardTitle>Issue / Return</CardTitle>
      ...
      <CardContent>
        <Table><TableHeader><TableRow><TableHead>Book</TableHead><TableHead>Member</TableHead><TableHead>Issue Date</TableHead><TableHead>Due Date</TableHead><TableHead>Return Date</TableHead><TableHead>Fine</TableHead><TableHead>Status</TableHead><TableHead>Action</TableHead></TableRow></TableHeader>
          <TableBody>{isLoading ? <TableRow><TableCell colSpan={8} className="text-center">Loading...</TableCell></TableRow> : issued?.length === 0 ? <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground">No records.</TableCell></TableRow> : issued?.map(i => (
            <TableRow key={i.id}><TableCell className="font-medium">{getBookTitle(i.bookId)}</TableCell><TableCell>{getMemberName(i.memberId, i.memberType)}</TableCell><TableCell>{i.issueDate}</TableCell><TableCell>{i.dueDate}</TableCell><TableCell>{i.returnDate || '-'}</TableCell><TableCell>{i.fineAmount ? `Rs.${i.fineAmount}` : '-'}</TableCell><TableCell><Badge variant={i.status === 'Issued' ? 'destructive' : 'default'}>{i.status}</Badge></TableCell><TableCell>{i.status === 'Issued' && <Button variant="outline" size="sm" onClick={() => handleReturn(i.id)}>Return</Button>}</TableCell></TableRow>
          ))}</TableBody></Table>
      </CardContent>
    </Card>
  );
