...
const COLLEGE_SHORT = "MKU Affiliated College";
const COLLEGE_LOCATION = "Tamil Nadu, India";
...
router.get("/print/fee-receipt/:paymentId", requireAuth, requirePermission("print_templates"), async (req, res): Promise<void> => {
  try {
    ...
    res.json({
      collegeName: COLLEGE_NAME, location: COLLEGE_LOCATION, university: UNIVERSITY,
      receipt: {
...
router.get("/print/mark-statement/:studentId", requireAuth, requirePermission("print_templates"), async (req, res): Promise<void> => {
  try {
    ...
    .orderBy(examsTable.semester, subjectsTable.code);
    ...
    let overallGrade = "F";
    if (overallPct >= 90) overallGrade = "O";
...
router.get("/print/fee-due-notice/:studentId", requireAuth, requirePermission("print_templates"), async (req, res): Promise<void> => {
  try {
    ...
    const pendingInstalments = await db.select({
      id: feeInstalmentsTable.id,
      instalmentNumber: feeInstalmentsTable.instalmentNumber,
      amount: feeInstalmentsTable.amount,
      dueDate: feeInstalmentsTable.dueDate,
      status: feeInstalmentsTable.status,
    })
    .from(feeInstalmentsTable)
    .where(and(eq(feeInstalmentsTable.studentId, studentId), eq(feeInstalmentsTable.status, "Pending")))
    .orderBy(feeInstalmentsTable.dueDate);
...
router.get("/print/provisional-certificate/:studentId", requireAuth, requirePermission("print_templates"), async (req, res): Promise<void> => {
  try {
    ...
    if (overallPct >= 60) classObtained = "First Class";
    else if (overallPct >= 50) classObtained = "Second Class";
